...
import { TimeLogTableSkeleton } from "../Skeletals/Project";
    ...
    try {
      ...
      if (response.data.success) {
        setTimeLogs((prevTimeLogs) =>
          prevTimeLogs.filter((timeLog) => timeLog._id.toString() !== timeLogId)
        );
          ...
          <tbody className="text-body-sm text-on-surface">
            {timeLogs?.map((timeLog) => (
              ...
              >
                ...
                <td className="p-3 ">
                  {(Math.floor(timeLog?.duration as number) / 3600).toFixed(2)} hours
                </td>
                ...
                <td className="p-3">
                  <span
                    onClick={() => {
                      setTimeLogId(timeLog._id.toString());
